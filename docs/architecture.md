...
同じ考え方を、APIの境界だけでなく**仕様と実装の間**にも広げています。仕様の受け入れ条件にIDを与え、実装とテストへマーカーで結び、片方だけが変更された状態を検出します。詳しくは [development-approach.md](development-approach.md#要求トレーサビリティ) を参照してください。
